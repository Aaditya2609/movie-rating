...
export const MovieProvider = ({ children }) => {
    ...
    useEffect(() => {
        const storedData = JSON.parse(localStorage.getItem('movie'));
    ...
    useEffect(() => {
        const storedData = JSON.parse(localStorage.getItem('starred'));
        if (storedData) {
            setStarred(storedData);
        } 
        else { 
    ...
    const updateLocalStorage = (newMovie) => {
        localStorage.setItem("movie", JSON.stringify(newMovie));
        setMovie(newMovie);
    }

    const updateLocalStorageStarred = (newMovie) => {
        localStorage.setItem("starred", JSON.stringify(newMovie));
        setStarred(newMovie);
    }

    const updateLocalStorageWatchLater = (newMovie) => {
        localStorage.setItem("watchlater", JSON.stringify(newMovie));
        setWatchLater(newMovie);
    }

    return (
        <MovieContext.Provider value={{movie,starred,watchLater,setMovie,updateLocalStorage,updateLocalStorageWatchLater,updateLocalStorageStarred}}>
            {children}
